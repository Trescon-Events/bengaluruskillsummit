...
function getEcosystemCategories(data) {
  let cats = [];
...
export default function EcosystemPartners2026() {
  const [categories, setCategories] = useState(() => getEcosystemCategories(STATIC_PARTNERS_DATA_2026));
  ...
  useEffect(() => {
    ...
    fetch(API_URL)
      ...
      .then((data) => {
        if (!isMounted) return;
        const cats = getEcosystemCategories(data);
        if (cats.length > 0) {
          setCategories(cats);
        }
        setLoading(false);
      ...
      .catch((err) => {
        ...
        // Do not display error to user if we have static fallback
        setLoading(false);
      });
